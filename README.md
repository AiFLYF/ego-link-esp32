...
板子**没有自己的 HTTP 服务端**（它是客户端），所以画面只能"推"上来：

```
板子 --POST /api/frame--> 服务器 --GET /api/frame--> 网页 <img>
```

- 固件 `camera_post_frame()` 抓一帧 JPEG **直接 POST**（不套 base64，省 33% 带宽）
- **按需推流**：`cam_stream` 指令开关，默认关。推一帧要占 WiFi 10~20 KB，
  而且 `camera_post_frame()` 是阻塞的（约 100 ms），常开会占住采样循环 ——
  所以做成按需开启，不用时零开销
- 网页只取"最近一帧"，约 5 帧/秒

**拍照**（`cam_capture` 指令）会同时做两件事：

| 存哪 | 位置 | 说明 |
|---|---|---|
| **板子 SD 卡** | `/sdcard/CAMxxxx.JPG` | 8.3 文件名；照片跟着板子走，拔卡就能取 |
| **服务器** | `server/data/shots/<设备>/` | 网页的照片列表用；每台上限 200 张，超了删最旧 |
| **浏览器** | IndexedDB | 用户要的"存浏览器、保持持久性"。**不用 localStorage**：照片是二进制，后者只存字符串且只有 5 MB |

删除：列表里每条都能删 —— 服务器上的走 `/api/shots/delete`，本机的走 IndexedDB。

**安全**：只收 JPEG 魔数（`FF D8`）的帧，别的字节直接 400；照片路径**夹紧**
（文件名/设备名里出现 `/` `\` `..` 一律拒）。两条都有回归测试。

> ⚠️ 摄像头自检失败时不会有画面。先看板子串口：
> `failed to detect DVP camera` 说明**传感器没响应**（FPC 排线/模组），
> 而不是配置问题 —— 同一 I2C 总线上的加速度计正常就能确认总线没问题。

### 存储管理（网页）

点「读取存储信息」（`sd_ls`）后会显示：

- **占用圆环**：中间写百分比，>75% 变琥珀、>90% 变红。**点圆环 = 滚到文件列表并高亮**
- **文件列表**：文件名 + 大小 + 每行一个「删除」（`sd_rm`，带二次确认）

命令走既有的**多选机制** —— 勾几台设备就一起读。卡里若是长文件名/中文名的旧文件，
在 `CONFIG_FATFS_LFN_NONE` 下板子**既看不见也删不掉**，只能用「格式化 SD 卡」
（`sd_format`）整卡格式化。
