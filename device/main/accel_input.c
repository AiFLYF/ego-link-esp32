...
/* Default sensor→screen map: o0 = out(−x, +z, −y)。
 * **这是 2026-09-23 用户在真机上亲眼确认正确的那个档位**：
 * 先用 out(+x,+z,−y) 时"上下对、左右反"，把 x 取反后两个方向都对。
 * 依据：这颗 SC7A20 竖装（y 轴 = 板子法线）且相对板面是镜像的。 */
#define ACCEL_ORIENT_DEFAULT 0
